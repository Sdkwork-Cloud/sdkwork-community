...
describe("CommunityService", () => {
  it("fails closed while the owner App API does not expose the mobile capability model", async () => {
    const community = {
      coverImage: "",
    ...
    for (const operation of operations) {
      await expect(operation()).rejects.toBeInstanceOf(CommunityCapabilityUnavailableError);
    }
  });
});
